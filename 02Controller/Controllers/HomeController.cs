...
namespace _02Controller.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            int[] score = { 78,99,20,20,100,66,64};
            string show = "";
            int sum = 0;

            foreach (var m in score) //拜訪(迴圈)
            {
                show += m + ",";
                sum += m;
            }
        ...
        public string ShowImageIndex(int index)
        {
            ...
            string[] name = { "櫻桃鴨", "鴨油高麗菜", "鴨油麻婆豆腐", "櫻桃鴨握壽司", "片皮鴨捲三星蔥", "三杯鴨", "櫻桃鴨片肉", "慢火白菜燉鴨湯" };   //字串陣列
        ...
        {
            int tmp = 0;
            for (int j = score.Length - 1; j >= 0; j--) //由大到小排, 內圈可以少算一圈
            {
        ...
        public string ShowSortAry2()
        {
            ...
            var s = from m in score       //score 丟到 m, 從from開始寫
